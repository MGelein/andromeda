...
class Andromeda{
  ...
  author:string;
  //the environment variable. This is the saved object.
  environment: Environment = new Environment();
  //The utility functions for this project
  util:AndromedaUtil = new AndromedaUtil();
  //Holds the UI changing methods
  ui:AndromedaUI = new AndromedaUI(this);
  //Holds the file interface methods
  file:AndromedaFile = new AndromedaFile(this);
  //Holds the externally accessible functions
  ext:AndromedaExternal = new AndromedaExternal();
  
  /**
   * Used to create the Andromeda object. Sets the author and version
   * @param {number} version the version of this program
   * @param {string} author 
   */
  constructor(version:number, author:string){
...
(function(_a : Andromeda){
  ...
  var textDropdownHTML:string = "<div class='row'><hr>"
   ...
   */
  function searchPerseus(btn:JQuery, field:JQuery, startFromScratch:boolean = true):void {
    //load the query from the UI and check its length
    var query:string = (field.val() as string).trim();
    field.val('');
   ...
   * urls
   */
  function startLoadingText(perseusUrl:string):void {
    ...
    $.ajax({url: perseusUrl, success : function(result){
        ...
        if(result.indexOf('<title>Perseus Search results</title>') != -1 || result.indexOf('<link href="/js/perseusld/perseusld.css" rel="stylesheet" type="text/css"/>') == -1){
          ...
          _a.environment.text.shortcut = jResult.find('#header_jump_form > input[name="doc"]').val() as string;
          ...
          while (true){
            ...
            var translation: AndromedaTranslation = new AndromedaTranslation(transHeader.textContent.split('\n')[3]);
   ...
   */
  var loadTranslation = function(html, title):void{
   ...
   */
  var loadTranslations = function():void{
  ...
  var selectionHandler = function(event){
    var sel:Selection = _a.util.getSelection();
    ...
    var positionAddNote = function():void{
  ...
  var loadNoteDisplay = function(note){
    ...
    $.each(locusParts, function(index:number, part:string){
        if(index == 0){
    ...
    $('.noteData').unbind('click').click(function(event){
        ...
        $(this).keydown(function(event){
          if(event.keyCode == 13 && !event.shiftKey){
            ...
            var noteId:number = parseFloat($(this).parent().attr('note-id'));
  ...
  var removeNote = function(btn){
    ...
    var i = 0; var max = _a.environment.notes.all.length; var n;
    for(i = 0; i < max; i++){
  ...
  var loadDictionaryCache = function(){
    ...
    $.each(_a.environment.dictionary.cache, function(index:number, word:AndromedaWord){
   ...
   */
  var loadDictionaryPerseus = function(url:string, title:string, locus:string, ignoreCache:boolean = false){
    //construct the link
    var morphUrl = MORPH_URL + url;
    //before loading the remote data, check if it is already cached
    var cached = false;
    $.each(_a.environment.dictionary.cache, function(index, word:AndromedaWord){
  ...
  var addDictNote = function(event, btn){
    ...
    var toAdd:AndromedaNote = new AndromedaNote(_a.environment.dictionary.current.locus);
    toAdd.type = 'dict';
  ...
  var cacheDictWord = function(title, html, url, locus){
    var word:AndromedaWord = new AndromedaWord();
    word.title = title;
    word.data = html;
    word.url = url;
    word.locus = locus;
  ...
  var registerClickHandlers = function(){
    ...
    $('#loadFromFileButton').change(function(event){
        _a.file.load((event.target as HTMLInputElement).files[0]);
    });
  }
